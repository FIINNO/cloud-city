...
function randomizePosition(camera){
    ...
    return new THREE.Vector3(newPositionX, newPositionY, 0);
}

function updateCloudCar(cloudCarObject, camera){
...
export function animateCloudCar(cloudCarObject, camera){
    ...
    const velocity = 4;
    if(cloudCarObject.position.distanceTo(new THREE.Vector3(0,0,0))>4000){
        cloudCarObject = updateCloudCar(cloudCarObject, camera);
        return cloudCarObject;
    }
    //console.log(cloudCarObject.position.distanceTo(0,0,0));
